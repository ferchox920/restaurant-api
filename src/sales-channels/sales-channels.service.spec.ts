...
describe('SalesChannelsService', () => {
  ...
  it('creates a sales channel', async () => {
    ...
    expect(result.subTaxes).toEqual([]);
  });

  it('creates a sales channel with sub taxes', async () => {
    prismaService.salesChannel.findUnique
      .mockResolvedValueOnce(null)
      .mockResolvedValueOnce(null);
    prismaService.salesChannel.create.mockResolvedValueOnce({
    ...
    const result = await service.create(
      {
        ...
        commissionValue: 0,
        subTaxes: [{ name: 'IVA', percentage: 21 }],
      },
      'manager-1',
    );
  ...
  it('rejects duplicated sub tax names', async () => {
    prismaService.salesChannel.findUnique
      .mockResolvedValueOnce(null)
      .mockResolvedValueOnce(null);
  ...
  it('normalizes legacy commission fields to NONE and 0', async () => {
    prismaService.salesChannel.findUnique
      .mockResolvedValueOnce(null)
      .mockResolvedValueOnce(null);
    prismaService.salesChannel.create.mockResolvedValueOnce({
  ...
  it('allows legacy commission values because sub taxes are authoritative', async () => {
    prismaService.salesChannel.findUnique
      .mockResolvedValueOnce(null)
      .mockResolvedValueOnce(null);
    prismaService.salesChannel.create.mockResolvedValueOnce({
